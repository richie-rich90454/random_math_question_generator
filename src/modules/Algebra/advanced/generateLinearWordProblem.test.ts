...
describe("generateLinearWordProblem", ()=>{
	...
	it("generates consecutive integers problem correctly", ()=>{
		...
		generateLinearWordProblem();
		expect(mockDiv.innerHTML).toBe("<div class=\"problem-text\">The sum of two consecutive integers is 15. Find the smaller integer.</div><div>\\[ \\text{Answer: } \\]</div>");
	...
	it("generates money problem correctly", ()=>{
		...
		generateLinearWordProblem();
		expect(mockDiv.innerHTML).toBe("<div class=\"problem-text\">You have 3 quarters and 6 dimes. How much money do you have in cents?</div><div>\\[ \\text{Answer: } \\]</div>");
		expect((window as any).correctAnswer).toEqual(expect.objectContaining({
			correct: "135",
			alternate: "135",
			display: "135"
	...
	it("generates distance problem correctly", ()=>{
		...
		generateLinearWordProblem();
		expect(mockDiv.innerHTML).toBe("<div class=\"problem-text\">A car travels at 35 mph for 4 hours. How far does it travel?</div><div>\\[ \\text{Answer: } \\]</div>");
		expect((window as any).correctAnswer).toEqual(expect.objectContaining({
			correct: "140",
			alternate: "140",
			display: "140"
	...
	it("generates age problem correctly", ()=>{
		...
		generateLinearWordProblem();
		expect(mockDiv.innerHTML).toBe("<div class=\"problem-text\">A person is 20 years old. How old were they 4 years ago?</div><div>\\[ \\text{Answer: } \\]</div>");
		expect((window as any).correctAnswer).toEqual(expect.objectContaining({
			correct: "16",
			alternate: "16",
			display: "16"
	...
	it("generates mixture problem correctly", ()=>{
		...
		generateLinearWordProblem();
		expect(mockDiv.innerHTML).toBe("<div class=\"problem-text\">A 20 gallon mixture contains 38% alcohol. How many gallons of alcohol are in it?</div><div>\\[ \\text{Answer: } \\]</div>");
		expect((window as any).correctAnswer).toEqual(expect.objectContaining({
			correct: "8",
			alternate: "8",
			display: "8"
